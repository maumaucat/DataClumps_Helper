...
public class DataClumpRefactoring implements LocalQuickFix {
    ...
    public void applyFix(@NotNull Project project, @NotNull ProblemDescriptor problemDescriptor) {
        ...
        List<Property> selectedProperties = PsiUtil.executeInEDTAndWait(dialog::getProperties);
        ...
        if (currentElement instanceof TypeScriptClass currentClass) {
            TypeScriptFunction constructor = (TypeScriptFunction) PsiUtil.runReadActionWithResult(currentClass::getConstructor);
            if (constructor != null) {
                getClassfieldDefiningParameter(constructor, currentDefiningParameters, currentDefinedClassFields);
            }
            getDefaultValues(currentClass, selectedProperties, currentDefaultValues);
        }
        if (otherElement instanceof TypeScriptClass otherClass) {
            TypeScriptFunction constructor = (TypeScriptFunction) PsiUtil.runReadActionWithResult(otherClass::getConstructor);
            if (constructor != null) {
                getClassfieldDefiningParameter(constructor, otherDefiningParameters, otherDefinedClassFields);
            }
            getDefaultValues(otherClass, selectedProperties, otherDefaultValues);
        }

        // create or use existing class
        TypeScriptClass extractedClass;
        // if the user selects to create a new class -> create the class
        if (Boolean.TRUE.equals(PsiUtil.executeInEDTAndWait(dialog::shouldCreateNewClass))) {

            String className = PsiUtil.executeInEDTAndWait(dialog::getClassName);
            PsiDirectory targetDirectory = PsiUtil.executeInEDTAndWait(dialog::getDirectory);
        ...
        } else { // if the user selects an existing class -> use the existing class
            extractedClass = PsiUtil.executeInEDTAndWait(dialog::getSelectedClass);
            assert extractedClass != null;
            ...
            CodeSmellLogger.info("Using existing class " + PsiUtil.getQualifiedName(extractedClass));

            // save the original parameters -> needed for refactoring the function calls
            List<Property> originalParameters = new ArrayList<>();
            TypeScriptFunction extractedConstructor = (TypeScriptFunction) PsiUtil.runReadActionWithResult(extractedClass::getConstructor);
            if (extractedConstructor != null) {
                originalParameters = getParametersAsPropertyList(extractedConstructor);
            }
            ...
            extractedConstructor = (TypeScriptFunction) PsiUtil.runReadActionWithResult(extractedClass::getConstructor);
            assert extractedConstructor != null;
            HashMap<Classfield, String> defaultValues = new HashMap<>();
            getDefaultValues(extractedClass, selectedProperties, defaultValues);

            refactorConstructorCalls(extractedConstructor, originalParameters, defaultValues);
        }
    ...
    private TypeScriptClass extractClass(PsiDirectory dir, String className, List<Property> fields) {
        ...
        for (Property field : fields) {
            ...
            if (DataClumpSettings.getInstance().getState().includeModifiersInExtractedClass != DataClumpSettings.Modifier.NONE && field instanceof Classfield && ((Classfield) field).isPublic())
                continue;
        ...
        VirtualFile virtualFile = PsiUtil.runReadActionWithResult(() -> dir.getVirtualFile().findChild(className + ".ts"));
        assert virtualFile != null;
        PsiFile file = PsiUtil.runReadActionWithResult(() -> PsiManager.getInstance(dir.getProject()).findFile(virtualFile));
        TypeScriptClass extractedClass = PsiUtil.runReadActionWithResult(() -> PsiTreeUtil.findChildOfType(file, TypeScriptClass.class));
    ...
    private void adjustConstructor(TypeScriptClass psiClass, List<Property> matchingProperties) {
        ...
        CodeSmellLogger.info("Adjusting constructor of " + PsiUtil.getQualifiedName(psiClass) + "...");

        TypeScriptFunction constructor = (TypeScriptFunction) PsiUtil.runReadActionWithResult(psiClass::getConstructor);
        ...
        if (constructor == null) {
            ...
            for (JSParameterListElement psiParameter : PsiUtil.runReadActionWithResult(() -> Objects.requireNonNull(constructor.getParameterList()).getParameters())) {
            ...
            for (Property property : matchingProperties) {
                ...
                if (property instanceof Classfield && (((Classfield) property).getModifiers().contains("abstract") || ((Classfield) property).getModifiers().contains("declare")))
                    continue;
            ...
            JSBlockStatement body = PsiUtil.runReadActionWithResult(constructor::getBlock);
    ...
    private void getClassfieldDefiningParameter(TypeScriptFunction function, HashMap<Parameter, Classfield> definingParameters, HashMap<Classfield, Parameter> definedClassfields) {
        ...
        // iterate over all parameters of the constructor
        for (JSParameterListElement psiParameter : PsiUtil.runReadActionWithResult(function::getParameters)) {
        ...
        for (Classfield classfield : classfields) {
            if (properties.contains(classfield)) {
                ...
                if (psiField instanceof TypeScriptField) {
                    initializer = PsiUtil.runReadActionWithResult(((TypeScriptField) psiField)::getInitializer);
                } else {
                    initializer = PsiUtil.runReadActionWithResult(((TypeScriptParameter) psiField)::getInitializer);
                }
                if (initializer != null) {
    ...
    private void addGetterAndSetter(TypeScriptClass psiClass, List<Property> properties) {
        for (Property property : properties) {
            ...
            if (classfield == null) {
                CodeSmellLogger.error("Field " + property.getName() + " not found in class " + PsiUtil.getQualifiedName(psiClass), new IllegalArgumentException());
                continue;
            }
            ...
            if (!PsiUtil.hasGetter(psiClass, classfield)) {
                ...
                if (!PsiUtil.getName(psiElement).startsWith("_")) {
                    PsiUtil.rename((PsiNamedElement) psiElement, "_" + PsiUtil.getName(psiElement));
                }
    ...
    private String getDefaultInit(TypeScriptClass psiClass, TypeScriptClass extractedClass, HashMap<Classfield, String> defaultValues) {
        ...
        List<Property> extractedParameters = getParametersAsPropertyList((TypeScriptFunction) Objects.requireNonNull(PsiUtil.runReadActionWithResult(extractedClass::getConstructor)));

        StringBuilder init = new StringBuilder();
        init.append("new ").append(PsiUtil.runReadActionWithResult(extractedClass::getName)).append("(");
    ...
    private void refactorElement(PsiElement element, TypeScriptClass extractedClass, List<Classfield> dataClump, HashMap<Classfield, Parameter> definedClassfields, HashMap<Classfield, String> defaultValues) {
        ...
        if (element instanceof TypeScriptClass psiClass && !Objects.equals(PsiUtil.getQualifiedName(psiClass), PsiUtil.getQualifiedName(extractedClass))) {
    ...
    private void addImport(PsiElement element, TypeScriptClass extractedClass) {
        ...
        PsiFile elementFile = PsiUtil.runReadActionWithResult(element::getContainingFile);
        PsiFile extractedFile = PsiUtil.runReadActionWithResult(extractedClass::getContainingFile);
        ...
        ApplicationManager.getApplication().runReadAction(() -> {
            ...
            for (ES6ImportDeclaration importStatement : PsiTreeUtil.findChildrenOfType(elementFile, ES6ImportDeclaration.class)) {
                if (importStatement.getNamedImports() != null && importStatement.getNamedImports().getText().contains(Objects.requireNonNull(extractedClass.getName()))) {
                    return;
                }
            }
        });
        ...
        String importStatement = "import { " + PsiUtil.getName(extractedClass) + " } from '" + relativePath + "';\n";
        PsiElement firstChild = PsiUtil.runReadActionWithResult(elementFile::getFirstChild);
    ...
    private void refactorClass(TypeScriptClass psiClass, TypeScriptClass extractedClass, List<Classfield> dataClump, HashMap<Classfield, Parameter> definedClassfields, HashMap<Classfield, String> defaultValues) {
        CodeSmellLogger.info("Refactoring class " + PsiUtil.getQualifiedName(psiClass) + "...");

        String fieldName = "my_" + PsiUtil.getName(extractedClass).toLowerCase(); // this should be a unique name

        // add the extracted class as a field
        ES6FieldStatementImpl newFieldStatement = PsiUtil.createJSFieldStatement(psiClass, fieldName, PsiUtil.runReadActionWithResult(() -> extractedClass.getJSType().getTypeText()), "public", new ArrayList<>(), getDefaultInit(psiClass, extractedClass, defaultValues));
    ...
    private void updateConstructor(TypeScriptClass psiClass, List<Classfield> dataClump, TypeScriptClass extractedClass, String fieldName, HashMap<Classfield, Parameter> definedClassfields, HashMap<Classfield, String> defaultValues) {
        ...
        CodeSmellLogger.info("Updating constructor of " + PsiUtil.getQualifiedName(psiClass) + "...");
        ...
        JSStatement initialization = JSPsiElementFactory.createJSStatement("this." + fieldName + " = " + fieldName + ";", psiClass);

        // add the initialization to the constructor block after the super call if it exists
        JSBlockStatement block = PsiUtil.runReadActionWithResult(constructor::getBlock);
        assert block != null;
        ...
        if (superCall != null) {
            ...
            insertPos = PsiUtil.runReadActionWithResult(block::getFirstChild);
        }
        WriteCommandAction.runWriteCommandAction(psiClass.getProject(), () -> {
    ...
    private void refactorConstructorParameter(TypeScriptFunction constructor, List<Classfield> dataClump, HashMap<Classfield, Parameter> definedClassfields, String newParameterName) {
        ...
        for (Classfield property : definedClassfields.keySet()) {
            ...
            for (PsiReference reference : PsiUtil.runReadActionWithResult(() -> ReferencesSearch.search(parameter))) {
                replaceReferenceWithGetter(PsiUtil.runReadActionWithResult(() -> PsiTreeUtil.getParentOfType(constructor, TypeScriptClass.class)), reference, newParameterName, property.getName(), false);
            }
    ...
    private void addClassAsParameter(TypeScriptFunction function, TypeScriptClass extractedClass, String newParameterName) {
        JSParameterList parameterList = PsiUtil.runReadActionWithResult(function::getParameterList);
    ...
    private void refactorConstructorCalls(TypeScriptFunction constructor, List<Property> originalParameters, HashMap<Classfield, String> defaultValues) {
        if (!PsiUtil.runReadActionWithResult(constructor::isConstructor)) return;
        ...
        for (PsiReference functionCall : PsiUtil.runReadActionWithResult(() -> ReferencesSearch.search(constructor))) {
            CodeSmellLogger.info("Refactoring constructor call... " + PsiUtil.runReadActionWithResult(() -> functionCall.getElement().getText()));
            ...
            for (JSParameterListElement currentPsiParameter : PsiUtil.runReadActionWithResult(constructor::getParameters)) {
                ...
                if (index != -1) {
                    currentValue = PsiUtil.runReadActionWithResult(originalArguments[index]::getText);
                } else {
            ...
            WriteCommandAction.runWriteCommandAction(constructor.getProject(), () -> {
                JSArgumentList newArguments = PsiUtil.createJSArgumentListFromText(argumentList, updatedArguments.toString());
     ...
     *                                   Only needed if the function is a constructor.
     */
    private void refactorFunctionCalls(TypeScriptFunction function, List<Property> originalParameters, TypeScriptClass extractedClass, List<Classfield> dataClump, @Nullable HashMap<Classfield, Parameter> originalDefinedClassfields, @Nullable HashMap<Classfield, String> defaultValues) {

        List<Property> extractedParameters = getParametersAsPropertyList((TypeScriptFunction) PsiUtil.runReadActionWithResult(extractedClass::getConstructor));
    ...
    private void refactorFunction(TypeScriptFunction psiFunction, TypeScriptClass extractedClass, List<Classfield> dataClump) {
        CodeSmellLogger.info("Refactoring function " + PsiUtil.getQualifiedName(psiFunction) + "...");

        String newParameterName = "my_" + PsiUtil.getName(extractedClass).toLowerCase(); // this should be a unique name
    ...
    private void refactorFunctionParameter(TypeScriptFunction function, List<Classfield> dataClump, String newParameterName) {
        ...
        for (JSParameterListElement parameter : PsiUtil.runReadActionWithResult(function::getParameters)) {
            ...
            if (dataClump.contains(currentParameter)) { // only refactor the extracted properties
                ...
                // replace all references to the parameter with getter calls on the extracted class
                for (PsiReference reference : PsiUtil.runReadActionWithResult(() -> ReferencesSearch.search(parameter))) {
                    JSExpression newExpression = JSPsiElementFactory.createJSExpression(newParameterName + "." + currentParameter.getName(), reference.getElement());
    ...
    private void replaceAssignmentWithSetter(TypeScriptClass psiClass, JSAssignmentExpression assignment, String fieldName, String propertyName) {
        ...
        if (PsiUtil.runReadActionWithResult(() -> PsiTreeUtil.getParentOfType(assignment, TypeScriptClass.class)) == psiClass) {
            expressionText = "this." + fieldName + "." + propertyName + " = " + Objects.requireNonNull(PsiUtil.runReadActionWithResult(() -> assignment.getROperand()).getText());
        } else {
            expressionText = Objects.requireNonNull(PsiUtil.runReadActionWithResult(() -> Objects.requireNonNull(assignment.getLOperand()).getFirstChild().getFirstChild().getText() + "." + fieldName + "." + propertyName + " = " + Objects.requireNonNull(assignment.getROperand()).getText()));
        }
    ...
    private void replaceReferenceWithGetter(TypeScriptClass psiClass, PsiReference reference, String fieldName, String propertyName, boolean isFieldReference) {
        String expressionText;
        if (PsiTreeUtil.getParentOfType(PsiUtil.runReadActionWithResult(reference::getElement), TypeScriptClass.class) == psiClass && isFieldReference) {
        ...
        WriteCommandAction.runWriteCommandAction(reference.getElement().getProject(), () -> {
            JSExpression newExpression = JSPsiElementFactory.createJSExpression(expressionText, PsiUtil.runReadActionWithResult(reference::getElement));
            reference.getElement().replace(newExpression);
            PsiDocumentManager.getInstance(reference.getElement().getProject()).commitAllDocuments();
        });
    }
    ...
    public Set<TypeScriptClass> getUsableClasses(List<Property> properties) {
        ...
        for (JSClass psiClass : potentialClasses) {
            ...
            if (!PsiUtil.runReadActionWithResult(psiClass::isValid) || PsiUtil.runReadActionWithResult(psiClass::getName) == null || !(psiClass instanceof TypeScriptClass))
                continue;
    ...
    private boolean hasConflictingSetter(TypeScriptClass psiClass, String fieldName) {
        ...
        for (JSFunction psiFunction : PsiUtil.runReadActionWithResult(psiClass::getFunctions)) {
            if (!psiFunction.isSetProperty()) continue;

            String setterName = PsiUtil.getName(psiFunction);
            ...
            if (setterName.equals(fieldName)) {
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    response.set(Messages.showYesNoDialog(psiFunction.getProject(), "<html><body>" + psiFunction.getText() + "<br><br>" + "</body></html>", "Can this function be used as a setter for the field </b> \"" + fieldName + "\"?", Messages.getQuestionIcon()));
                });
    ...
    private boolean hasConflictingGetter(TypeScriptClass psiClass, String fieldName) {
        ...
        for (JSFunction psiFunction : PsiUtil.runReadActionWithResult(psiClass::getFunctions)) {
            if (!PsiUtil.runReadActionWithResult(psiFunction::isGetProperty)) continue;

            String getterName = PsiUtil.getName(psiFunction);
    ...
    private static class DefaultValues {
        ...
        public static String getDefaultValue(Property property) {
            ...
            ApplicationManager.getApplication().invokeLater(() -> {
                do {
                    input.set(Messages.showInputDialog("<html>Please enter a (default) initialization for <b>" + property.getName() + " : " + property.getTypesAsString() + "</b>:<br></html>", "", Messages.getQuestionIcon()));
